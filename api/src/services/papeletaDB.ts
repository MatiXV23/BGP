...
export class PapeletaDB extends BaseDBRepository<Papeleta> {
  ...
  async getAll(): Promise<Papeleta[]> {
    try {
      ...
      return rows.map((row) => ({
        ...
        es_lista: Boolean(row.es_lista),
        descripcion: row.descripcion ?? undefined,
        color: row.color ?? undefined,
        id_partido: row.id_partido ?? undefined,
        organo_candidatura: row.organo_candidatura ?? undefined,
        id_departamento: row.id_departamento ?? undefined,
      })) as Papeleta[];
    } catch (error) {
      console.error("Error en PapeletaDB.getAll:", error);
  ...
  async getById(id: number): Promise<Papeleta> {
    try {
      ...
      console.error("Error en PapeletaDB.getById:", err);
      throw new PC_InternalServerError();
    }
  }
    ...
    try {
      ...
      if (err?.code === "ER_DUP_ENTRY")
        throw new PC_BadRequest(
          "Ya existe una papeleta con ese número de lista",
        );
      if (err?.code === "ER_NO_REFERENCED_ROW_2")
        throw new PC_BadRequest(
          "La elección, el partido o el departamento indicado no existe",
        );
      throw new PC_InternalServerError();
    }
  }
  ...
  async update(id: number, data: Partial<Papeleta>): Promise<void> {
    ...
    if (data.id_eleccion !== undefined) {
      fields.push("id_eleccion = ?");
      values.push(data.id_eleccion);
    }
    if (data.numero_lista !== undefined) {
      fields.push("numero_lista = ?");
      values.push(data.numero_lista);
    }
    if (data.es_lista !== undefined) {
      fields.push("es_lista = ?");
      values.push(data.es_lista);
    }
    ...
    if (data.id_partido !== undefined) {
      fields.push("id_partido = ?");
      values.push(data.id_partido);
    }
    if (data.organo_candidatura !== undefined) {
      fields.push("organo_candidatura = ?");
      values.push(data.organo_candidatura);
    }
    if (data.id_departamento !== undefined) {
      fields.push("id_departamento = ?");
      values.push(data.id_departamento);
    }
