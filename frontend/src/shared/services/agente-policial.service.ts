...
export class AgentePolicialService {
  ...
  private readonly base = `${environment.apiUrl}/agente-policial`;
  ...
  public async create(data: AgentePolicial): Promise<AgentePolicial> {
    return await firstValueFrom(
      this.httpClient.post<AgentePolicial>(this.base, data)
    );
  }
  ...
  public async update(
    ...
    await firstValueFrom(
      this.httpClient.put<void>(`${this.base}/${cedula_identidad}`, data)
    );
  }

  public async patch(
    cedula_identidad: number,
    data: Partial<AgentePolicial>
  ): Promise<void> {
    await firstValueFrom(
      this.httpClient.patch<void>(`${this.base}/${cedula_identidad}`, data)
    );
  }

  public async delete(cedula_identidad: number): Promise<void> {
    await firstValueFrom(
      this.httpClient.delete<void>(`${this.base}/${cedula_identidad}`)
    );
  }
}