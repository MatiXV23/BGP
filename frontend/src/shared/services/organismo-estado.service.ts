...
export class OrganismoEstadoService {
  ...
  private readonly base = `${environment.apiUrl}/organismo-estado`;

  public async getAll(): Promise<OrganismoEstado[]> {
    return await firstValueFrom(
      this.httpClient.get<OrganismoEstado[]>(this.base)
    );
  }

  public async getById(id: number): Promise<OrganismoEstado> {
    return await firstValueFrom(
      this.httpClient.get<OrganismoEstado>(`${this.base}/${id}`)
    );
  }

  public async create(data: OrganismoEstadoPost): Promise<OrganismoEstado> {
    return await firstValueFrom(
      this.httpClient.post<OrganismoEstado>(this.base, data)
    );
  }
